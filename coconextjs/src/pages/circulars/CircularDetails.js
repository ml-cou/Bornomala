...
const CircularDetails = ({ university }) => {
  const { t } = useTranslation();

  if (!university) return null;

  const defaultThumbnail = process.env.NEXT_PUBLIC_LOGO_DEFAULT_THUMBNAIL;
  const imageUrl = university.logo_url
    ? `${process.env.NEXT_PUBLIC_API_BASE_URL}${university.logo_url}`
    : defaultThumbnail;
      ...
      <div className="row">
        <div className="col-md-6">
          <ul className="list-group list-group-flush">
            ...
            <li className="list-group-item">
              <strong>{t("Platform Address")}:</strong>{" "}
              <a
                href={`${process.env.NEXT_PUBLIC_MAIN_URL}/org/${slugify(
                  university.name,
                  { lower: true }
                )}`}
                target="_blank"
              ...
              >
                {process.env.NEXT_PUBLIC_MAIN_URL}/org/
                {slugify(university.name, { lower: true })}
              </a>
            </li>
            <li className="list-group-item">
            ...
            <li className="list-group-item">
              ...
              <a
                href={university.web_address}
                target="_blank"
                rel="noopener noreferrer"
              >
                {university.web_address}
              </a>
            </li>
            <li className="list-group-item">
            ...
            <li className="list-group-item">
              ...
              >
                {university.status ? t("Active") : t("Inactive")}
              </span>
            </li>
          </ul>
        </div>
