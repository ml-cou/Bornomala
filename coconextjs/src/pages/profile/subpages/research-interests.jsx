// pages/profile/subpages/research-interests.jsx
import {
    React,
    useRef,
    useState,
...
const ResearchInterests = () => {
    ...
    const schema = yup.object().shape({
        research_interests: yup.array().of(
            yup.object().shape({
                label: yup.string().required(t('Research Interest is required')),
                value: yup.string().required(t('Research Interest is required')),
            })
        ).min(1, t('Research Interest is required')),
    });
    ...
    const fetchSavedOptions = async () => {
        try {
            const response = await executeAjaxOperationStandard({
                url: `${process.env.NEXT_PUBLIC_API_ENDPOINT_USER_RESEARCH_INTEREST_INFO_DETAILS}`,
                method: 'get',
                token,
            ...
            if (response.data && response.data.status) {
                if (response.data.status === 'success') {
                    const savedOptions = response.data.data.map(item => ({
                        label: item.research_interests_option.topic,
                        value: item.research_interests_option.id,
                    }));
                    setSelectedOptions(savedOptions);
                    setOriginalOptions(savedOptions); // Save the original options
                    setValue('research_interests', savedOptions);
                }
                setGlobalError('');
                setSuccessMessage('');
            } else {
    ...
    const onSubmit = async (formData) => {
        try {
            const researchInterestsData = formData.research_interests.map(item => ({
                research_interests_option_id: item.value
            }));
            setLoading(true);
            const response = await executeAjaxOperationStandard({
                url: process.env.NEXT_PUBLIC_API_ENDPOINT_USER_RESEARCH_INTEREST_INFO_DETAILS,
    ...
    const loadOptions = async (inputValue) => {
        try {
            ...
            if (response.data && response.data.status) {
                setGlobalError('');
                setSuccessMessage('');
                if (response.data.data.research_interests) {
                    return response.data.data.research_interests.map(item => ({ label: item.topic, value: item.id }));
                }
            } else {
                setSuccessMessage('');
                setGlobalError(response.message || t('An error occurred while fetching research options.'));
            }
        } catch (error) {
            return [];
        }
    ...
    const handleChange = (selected) => {
        ...
        setValue('research_interests', selected || []);
    };
                    ...
                    {isEditMode ? (
                        <form ref={formRef} onSubmit={handleSubmit(onSubmit)}>
                            <div className="mb-3">
                                ...
                                <AsyncSelect
                                    ...
                                    className={`react-select ${errors.research_interests ? 'is-invalid' : ''}`}
                                />
                                {errors.research_interests && <div className="text-danger">{errors.research_interests.message}</div>}
                            </div>
                        ...
                        <div>
                            ...
                            <div className="col-sm-12">
                                <p><strong>{t('Research Interests')}: </strong>
